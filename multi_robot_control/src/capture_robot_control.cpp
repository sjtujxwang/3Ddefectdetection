...
public:
    ROBOT_CONTROL()
    {
        ...
        ur_control_pub = nh.advertise<std_msgs::String>("/ur2/ur_driver/URScript", 1000);
        robot_capture_server = nh.advertiseService("robot_capture_service", &ROBOT_CONTROL::robot_capture_callback, this);
        ros::Duration(1.0).sleep();
        get_eef_pose();
    }

private:
    bool robot_capture_callback(multi_robot_control::robot_control::Request& req, multi_robot_control::robot_control::Response& res);
...
{
    ros::init(argc, argv, "capture_robot_control");
...
void ROBOT_CONTROL::get_eef_pose()
{
    ...
    while (ros::ok())
    {
        ...
        {
            // 坐标系的名称需要根据实际情况修改
            transformStamped_ur = tfBuffer.lookupTransform("ur10_base", "ur10_tool0_controller",ros::Time(0));
        }
...
bool ROBOT_CONTROL::robot_capture_callback(multi_robot_control::robot_control::Request& req, multi_robot_control::robot_control::Response& res)
{
    double a = 0.5;
    double v = 0.4;
