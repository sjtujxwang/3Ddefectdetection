...
std::vector<float> process(pcl::PointCloud<pcl::PointXYZ>::Ptr origin_cloud)
{
	...
	else if (Config::get<bool>("PASS_filter") && !Config::get<bool>("VoxelGrid_filter"))
	{
		...
		for (int i = 1; i <= num_cluster; i++)
		{
			...
			{
				//add transformed point cloud data, defect cluster n
				std::cout<<"xc,yc,zc: "<<xc<<" "<<yc<<" "<<zc<<std::endl;
				...
				std::cout<<"normal_estimate_area_ratio: "<<Config::get<float>("normal_estimate_area_ratio")<<std::endl; 
	...
	std::cout<<"defect[] before transformed: "<<std::endl;
	...
	std::cout<<"defect[] after transformed: "<<std::endl;
	for (int i = 0; i < defect.size(); ++i)
	{
		std::cout<<defect[i]<<std::endl;
	}
